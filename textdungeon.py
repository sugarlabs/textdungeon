    ...
    if 1 == 1:  # only to maintain indents
        ...
        if kbin[0] == 'f':
            if lockedahead(self.loc, self.doors, self.direction) == 0:
                ...
                if rdat:
                    ...
                    self.printtobuf(
                        'You cannot move forward, the way ahead is blocked')
        ...
        if kbin[0] == 'p':
            ...
            for i in lookfloor(self.loc, self.items):
                if kbin[2:] == i:
                    ...
                    self.items.remove(pickuponeitem(
                        self.loc, self.items, kbin[2:]))
                    self.inventory.append(kbin[2:])
                    pickedsomething = 1
                    self.printtobuf(
                        'Your backpack contains ' + str(self.inventory))
        ...
        if kbin[0] == 'd':
            if (kbin[2:]) in self.inventory:
                ...
                self.printtobuf('Your backpack contains ' +
                                str(self.inventory))
                self.items.append([self.loc[0], self.loc[1], kbin[2:]])
            else:
                self.printtobuf('You do not have ' +
                                kbin[2:] + 'in your backpack')
            ...
            if (kbin[2:]) in self.inventory:
                if rdat:
                    if rdat[0] != kbin[2:]:
                        ...
                        unlockindex = getunlockindex(
                            self.loc, self.doors, self.direction)
                        if unlockindex:
                            self.doors.remove(unlockindex)
                            self.doors.append(
                                [self.loc[0], self.loc[1], self.direction, 0])
                else:
                    self.printtobuf('You cannot use ' + kbin[2:] + 'here')
            else:
                self.printtobuf('You do not have ' +
        ...
        if kbin != '':
            rdat = getroomdata(self.loc, self.roomdata, self.direction)
            self.printtobuf(
                'You are in a dimly lit cavern facing ' +
                compass(
                    self.direction))
            self.printtobuf('On the floor is' +
                            str(lookfloor(self.loc, self.items)))
            if lockedahead(self.loc, self.doors, self.direction) == 0:
                if rdat:
                    ...
                    self.printtobuf('the way ahead is open')
            else:
                if rdat:
                    self.printtobuf(rdat[2])
                else:
